 ...
 * @author manhao/tobi
 */
public class SeeBetter30HardwareInterface extends CypressFX2Biasgen {

    /** The USB product ID of this device */
    static public final short PID = (short) 0x840B;
    static public final short DID = (short) 0x0003;

    /** Creates a new instance of CypressFX2Biasgen */
    public SeeBetter30HardwareInterface(int devNumber) {
        super(devNumber);
    }
    ...
    synchronized public void setPowerDown(boolean powerDown) throws HardwareInterfaceException {
        if(chip!=null && chip instanceof SeeBetter30){
            SeeBetter30config sb=(SeeBetter30config)chip.getBiasgen();
                        ...
                        switch (code) {
                            case 0: // address
                                ...
                                if ((eventCounter >= aeBufferSize) || (buffer.overrunOccuredFlag)) {
                                    ...
                                    if ((buf[i + 1] & triggerMask) == triggerMask) { 
...
//                                        {
//                                            doubleY=false;
//                                            System.out.println(yonlycons+ " Y addresses consecutively recieved in SeeBetter30HardwareInterface, total y only: "+ yonlycount); // this printout makes display very jerky!!!
//                                            yonlycons=0;
//                                        }
                                    } else {// y address
                                        // lasty = (0xFF & buf[i]); //
//                                        if (gotY) {// TODO creates bogus event to see y without x. This should not normally occur.
//                                  //          addresses[eventCounter] = (lasty << SeeBetter30.YSHIFT) + (SeeBetter30.SIZEX_TOTAL - 1 << 1);                 //(0xffff&((short)buf[i]&0xff | ((short)buf[i+1]&0xff)<<8));
