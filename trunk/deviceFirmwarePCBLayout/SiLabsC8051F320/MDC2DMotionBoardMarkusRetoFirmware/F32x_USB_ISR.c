...
// Release 1.4
//	  -All changes by TR
//	  - Modified vendor requests to serve for the MDC2D chip
...
extern bit doReset, doStream, hostInRequest, putVrecep, putLmc1, putLmc2;
extern char onChipBiases[];
extern char controlRegister;


extern void setBiasV( unsigned char msb, unsigned char lsb, unsigned char address);
extern void writeOnChipBiases(void);
extern void delay();
...
void Usb_ISR(void) interrupt 8         // Top-level USB ISR
{
   ...
   POLL_READ_BYTE(OUT1INT, bOut);
   {
      ...
      if (bIn & rbEP0 && !nextIsDataStage ){                 // Handle Setup packet received                               // or packet transmitted if Endpoint 0
         Handle_Setup();               // is transmit mode
      }
      if (bIn & rbIN1){                 // Handle In Packet sent, put new data                                // on endpoint 1 fifo
         Handle_In1();
      }
      if (bOut & rbOUT2){               // Handle Out packet received, take data                         // off endpoint 2 fifo
         Handle_Out2();
      }
   ...
   POLL_WRITE_BYTE(INDEX, 1);  //In case the main loop is comitting to the fifo.
}
      ...
      {                                // although if EP0 is idle, this should always be the case
         ...
         {                             // standard request
            ...
            case SET_INTERFACE:
               Set_Interface();
               break;
            case VENDOR_REQUEST_SEND_BIAS: //The host wants to set a bias value
			   setBiasV(Setup.wValue.c[MSB], Setup.wValue.c[LSB], Setup.wIndex.c[LSB]);
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
			   break;
			case VENDOR_REQUEST_SEND_ONCHIP_BIAS: // the host wants to send data to the on chip biasgenerator 
			   {
			   	onChipBiases[Setup.wValue.c[MSB]*3]= Setup.wValue.c[LSB];
				onChipBiases[Setup.wValue.c[MSB]*3+1]= Setup.wIndex.c[MSB];
				onChipBiases[Setup.wValue.c[MSB]*3+2]= Setup.wIndex.c[LSB];
				writeOnChipBiases();
		   	    POLL_WRITE_BYTE(E0CSR, rbSOPRDY);//setup_handled = TRUE;
			   }
			   break;
            case VENDOR_REQUEST_SET_DATA_TO_SEND:
               //putVrecep = Setup.wValue.c[LSB] | DESCR_FLAG_VRECEP; 
               //putLmc1 = Setup.wValue.c[LSB] | DESCR_FLAG_VLMC1;      
               //putLmc2 = Setup.wValue.c[LSB] | DESCR_FLAG_VLMC2;        
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
			   break;
            case VENDOR_REQUEST_START_STREAMING: // The host wants to start streaming.
			   doStream=1;
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
               break;
			case VENDOR_REQUEST_STOP_STREAMING: //The host wants to stop streaming.
			   doStream=0;
			   doReset=1;
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
               ...
               break;
			case VENDOR_REQUEST_SET_POWERDOWN_STATE:
			   pd=Setup.wValue.i;
			   POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
               break;
			case VENDOR_REQUEST_SET_DATA_TO_SEND_MDC2D:
			   controlRegister=(char)Setup.wValue.i;
			   writeOnChipBiases();
			   POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
			   break;
            default:
