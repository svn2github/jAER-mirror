...
xdata int g_iMainDataIndex = 0;		/*** forward - only index used as a placed holder in the data array ***/
xdata int g_iRepeatIndex = 0;		    /*** Used to point to the location of REPEAT data ***/
xdata int g_iTDIIndex = 0;			/*** Used to point to the location of TDI data ***/
xdata int g_iTDOIndex = 0;			/*** Used to point to the location of TDO data ***/
xdata int g_iMASKIndex = 0;			/*** Used to point to the location of MASK data ***/
...
xdata int  g_iLoopIndex = 0;			
xdata int  g_iLoopMovingIndex = 0;	/*** Used to point to the location of LOOP data ***/
xdata int  g_iLoopDataMovingIndex = 0;

xdata unsigned short g_usLCOUNTSize	= 0;
xdata unsigned char  g_ucLDELAYState = IDLE;
xdata unsigned short int  g_ucLDELAYTCK = 0;
xdata unsigned short int  g_ucLDELAYDelay = 0;
xdata unsigned short int m_loopState = 0;
...
#define	I2C_EEPR 0x51 //adress is 0101_0001

void EEPROMRe(unsigned short addr, unsigned char length, unsigned char *buf)
{
	unsigned char		i = 0;
	unsigned char		j = 0;
	...
	EZUSB_WriteI2C(I2C_EEPR, i, ee_str);

	for(j=0; j < length; j++)
		*(buf+j) = 0xcd;

	EZUSB_ReadI2C(I2C_EEPR, length, buf);
}
...
unsigned char GetByte(int a_iCurrentIndex, char a_cAlgo)
{
	...
	*************************************************************/
		...
		EEPROMRe(a_iCurrentIndex+EEPROM_CPLDCODE_START, 1, &ucData); /// data needs to be retrieved from EEPROM
	}
