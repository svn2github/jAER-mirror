...
*/
					...
					cpldSRBytes[7]&= ~0x80;  // clear lsb of first byte, which is yBit (bits are written big endian, so bit 63 is actually bit0 host side)
					sendCPLDState();
					...
					cpldSRBytes[7]|= 0x80;  // set lsb of first byte, which is yBit
					sendCPLDState();
