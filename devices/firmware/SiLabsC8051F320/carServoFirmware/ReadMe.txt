...
copyright Tobi Delbruck Jan/July 2007

Holds firmware for servo board specialized to control the speed and steering servos on an RC car, along
with merging radio receiver output with computer output.

Holds servomotor control project for SiLabs ServoUSB PCB.
 
The ServoUSB.wsp project is a full-blown USB driver/firmware combo that
uses the Thesycon driver on the host side. It goes with the class
SiLabsC8051F320_USBIO_ServoController on the host side.

NOTE: This project is built with the SiLabs IDE v2.8x. It must be built in a folder
