...
void caerDaemonize(void) {
	...
	// Handle errors first.
	if (result == -1) {
		caerLog(LOG_CRITICAL, "Daemonize", "Failed the first fork.");
		exit(EXIT_FAILURE);
	}
	...
	// Handle errors first.
	if (result == -1) {
		caerLog(LOG_CRITICAL, "Daemonize", "Failed the second fork.");
		exit(EXIT_FAILURE);
	}
	...
	if (chdir("/") != 0) {
		caerLog(LOG_CRITICAL, "Daemonize", "Failed to change directory to '/'.");
		exit(EXIT_FAILURE);
	}
	...
	if (open("/dev/null", O_RDONLY) != 0) {
		caerLog(LOG_CRITICAL, "Daemonize", "Failed to redirect stdin to log file.");
		exit(EXIT_FAILURE);
	}
	if (dup2(3, 1) != 1) {
		caerLog(LOG_CRITICAL, "Daemonize", "Failed to redirect stdout to log file.");
		exit(EXIT_FAILURE);
	}
	if (dup2(3, 2) != 2) {
		caerLog(LOG_CRITICAL, "Daemonize", "Failed to redirect stderr to log file.");
		exit(EXIT_FAILURE);
	}
