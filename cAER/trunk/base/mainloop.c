...
void caerMainloopRun(struct caer_mainloop_definition (*mainLoops)[], size_t numLoops) {
	if (numLoops == 0) {
		...
		caerLog(LOG_CRITICAL, "The number of Mainloops to start is specified at zero: nothing to start!");
		return;
	}
	...
	if (sigaction(SIGTERM, &shutdown, NULL) == -1) {
		caerLog(LOG_CRITICAL, "Failed to set signal handler for SIGTERM. Error: %s (%d).", caerLogStrerror(errno),
		errno);
		exit(EXIT_FAILURE);
	}

	if (sigaction(SIGINT, &shutdown, NULL) == -1) {
		caerLog(LOG_CRITICAL, "Failed to set signal handler for SIGINT. Error: %s (%d).", caerLogStrerror(errno),
		errno);
		exit(EXIT_FAILURE);
	}
	...
	if (mainloopThreads.loopThreads == NULL) {
		caerLog(LOG_CRITICAL, "Failed to allocate memory for main-loops. Error: %s (%d).", caerLogStrerror(errno),
		errno);
		exit(EXIT_FAILURE);
	}
	...
	for (size_t i = 0; i < mainloopThreads.loopThreadsLength; i++) {
		...
		if ((errno = pthread_create(&mainloopThreads.loopThreads[i].mainloop, NULL, &caerMainloopRunner,
			...
			caerLog(LOG_CRITICAL, "Failed to create main-loop %" PRIu16 " thread. Error: %s (%d).",
	...
	for (size_t i = 0; i < mainloopThreads.loopThreadsLength; i++) {
		if ((errno = pthread_join(mainloopThreads.loopThreads[i].mainloop, NULL) != 0)) {
			caerLog(LOG_CRITICAL, "Failed to join main-loop %" PRIu16 " thread. Error: %s (%d).",
				mainloopThreads.loopThreads[i].mainloopID, caerLogStrerror(errno), errno);
			// TODO: better cleanup on failure?
			exit(EXIT_FAILURE);
		}
...
sshsNode caerMainloopGetSourceInfo(uint16_t source) {
	...
	if (moduleData == NULL) {
		...
		caerLog(LOG_CRITICAL, "Impossible to get module data for source ID %" PRIu16 ".", source);
		pthread_exit(NULL);
	}
