...
static void caerInputDAViSFX2Run(caerModuleData moduleData, size_t argsNumber, va_list args);
...
static struct caer_module_functions caerInputDAViSFX2Functions = { .moduleInit = &caerInputDAViSFX2Init, .moduleRun =
	&caerInputDAViSFX2Run, .moduleConfig = NULL, .moduleExit = &caerInputDAViSFX2Exit };
...
static void allocateDataTransfers(davisFX2State state, uint32_t bufferNum, uint32_t bufferSize);
static void deallocateDataTransfers(davisFX2State state);
static void LIBUSB_CALL libUsbDataCallback(struct libusb_transfer *transfer);
static void dataTranslator(davisFX2State state, uint8_t *buffer, size_t bytesSent);
...
static void caerInputDAViSFX2ConfigListener(sshsNode node, void *userData, enum sshs_node_attribute_events event,
	const char *changeKey, enum sshs_node_attr_value_type changeType, union sshs_node_attr_value changeValue);

static inline void freeAllPackets(davisFX2State state) {
...
static void LIBUSB_CALL libUsbDataCallback(struct libusb_transfer *transfer) {
	davisFX2State state = transfer->user_data;
...
#define CHECK_MONOTONIC_TIMESTAMP(CURR_TS, LAST_TS) \
	if (CURR_TS <= LAST_TS) { \
		caerLog(LOG_ALERT, \
			"DAViSFX2: non-monotonic time-stamp detected: lastTimestamp=%" PRIu32 ", currentTimestamp=%" PRIu32 ", difference=%" PRIu32 ".", \
		...
		if ((event & 0x8000) != 0) {
			...
			switch (code) {
				...
				case 5: // Misc 8bit data, used currently only
						// for IMU events in DAViS FX2 boards
					break;
...
static libusb_device_handle *deviceOpen(libusb_context *devContext, uint8_t busNumber, uint8_t devAddress) {
	...
	if (result >= 0) {
		...
		for (size_t i = 0; i < (size_t) result; i++) {
			...
			if (devDesc.idVendor == DAVIS_FX2_VID && devDesc.idProduct == DAVIS_FX2_PID
				&& (uint8_t) ((devDesc.bcdDevice & 0xFF00) >> 8) == DAVIS_FX2_DID_TYPE) {
