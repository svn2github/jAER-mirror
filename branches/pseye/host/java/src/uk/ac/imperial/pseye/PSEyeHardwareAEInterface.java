 ...
 * 
 */
public class PSEyeHardwareAEInterface extends PSEyeCamera implements AEMonitorInterface {
    ...
    private CameraFramePacketRaw packet = new CameraFramePacketRaw(320 * 240);

    public PSEyeHardwareAEInterface(int cameraIndex) {
        super(cameraIndex);
    }
    ...
    public AEPacketRaw acquireAvailableEventsFromDriver() throws HardwareInterfaceException {
        ...
        int nframes = stream.available();
        if (nframes == 0) return null;
        
        int frameSize = stream.frameSize;
        packet.ensureCapacity(nframes * frameSize);
        packet.nFrames = 0;
        
        // loop across available frames
        while (packet.nFrames < nframes) {
            getFrame(packet.getAddresses(), packet.nFrames * frameSize);
            packet.getTimestamps()[packet.nFrames * frameSize] = (int) (stream.getLastTimeStamp() - startTimeUs);
            packet.nFrames++;
