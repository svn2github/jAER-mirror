...
Modified July 2006 for ServoMotor control by tobi.
...
//#define SEVILLA_BOARD
	...
	#define CMD_SETSERVO 7 // servo motor command, defined in JNI for SiLabs
...
xdata unsigned char pwmNumber, pwml1, pwmh1, pwml2, pwmh2;
...
// pwm interrupt vectored when there is a match interrup for PCA: only then do we change PCA compare register
...
//	BYTE msg;
	...
	BYTE	receivedMsg[4];	// rcvd msg buffer, for later use in host commands

	if (INTVAL	&	RX_COMPLETE)				//	RX Complete, assume we should send data
	{
		Block_Read(&receivedMsg,4);	// on rcv, read all the data
									// we don't do anything with it now

		if(receivedMsg[0]==CMD_SETSERVO)
		{
			LedRedOn();
			pwmNumber=receivedMsg[1];
			switch(pwmNumber)
			{
			// big endian 16 bit value to load into PWM controller
				case 0:
				{ // servo0
					pwmh1=receivedMsg[2]; // store the PCA compare value for later interrupt to load
					pwml1=receivedMsg[3];


					PCA0CPM1|=0x09; // enable match and interrupt for match for pca
				//	PCA0CN|=0x80; // enable PCA overflow interrupt
				...
				{ // servo1
					pwmh2=receivedMsg[2];
					pwml2=receivedMsg[3];
					PCA0CPM2|=0x09; // enable enable match and interrupt for match for pca
				}
			}			
...
//    TH0 = 0xFF-23; 	    // Timer 0 High Byte, reload value. This is FF-23 so timer0 takes 24 cycles = 1us to roll over. 
    TH0 = 0xFF-1; 	    // Timer 0 High Byte, reload value. This is FF-n so timer0 takes n+1 cycles = to roll over, time is (n+1)/24MHz  
	...
	// PCA0 is used for timestamping. neg edge on cex0 captures PCA0 value which is timestamp
	PCA0MD|=0x84;	// use timer0 overflow to clock PCA counter. leave wdt bit undisturbed. turn off PCA in idle.
	PCA0CPM0=0x10;	// negative edge on CEX0 captures PCA, which is req from sender. This makes timestamp.
	...
	// pca is 16 bit = 65k counts = 65ms pulse rate= 1/65ms = ~15.3Hz. 16 bit count varies pulse width. PCA value defines low time, therefore pulse width
...
//#define SERVO_DEFAULT 
	PCA0CPL1=0xff-100; // start with 100us pulse
	PCA0CPH1=0xff;
	PCA0CPL2=0xff-100; // start with 100us pulse
	PCA0CPH2=0xff;
...
#ifdef INI_BOARD
//    P0SKIP = 0x01;  //  0000 0001 Port 0 Crossbar Skip Register. Skip first pin so that bit 1 (req) P0.1 becomes CEX0 input to PCA0 capture module
