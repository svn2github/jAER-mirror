...
typedef union {unsigned char c[10];} BIASBYTES;
	...
	typedef code struct
{
   ...
   WORD wIndex;                        // varies according to request
   WORD wLength;                       // Number of bytes to transfer
} dat_buffer;                        // End of Setup Packet Type

	typedef code struct
{
   BYTE b1;                 // Request recipient, type, direction
   BYTE b2;                      // Specific standard request number
   BYTE b3;                        // varies according to request
   BYTE b4;                        // varies according to request
   BYTE b5;
   BYTE b6;
   BYTE b7;
   BYTE b8;

} data_buffer;                        // End of Setup Packet Type
