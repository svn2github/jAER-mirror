...
extern bit doReset, doStream, hostInRequest, putVx, putVy, putSvx, putSvy, putSph0;
      ...
      {                                // although if EP0 is idle, this should always be the case
         ...
         {                             // standard request
            ...
            case SET_INTERFACE:
               Set_Interface();
               break;
            case VENDOR_REQUEST_SEND_BIAS:
			   setBiasV(Setup.wValue.c[MSB], Setup.wValue.c[LSB], Setup.wIndex.c[LSB]); //'Byte order??
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
			   break;
            case VENDOR_REQUEST_SET_DATA_TO_SEND:
               putVx = Setup.wValue.c[LSB] | DESCR_FLAG_VX;   //' The right byte?? or MSB?
               putVy = Setup.wValue.c[LSB] | DESCR_FLAG_VY;      
               putSvx = Setup.wValue.c[LSB] | DESCR_FLAG_SVX;     
               putSvy = Setup.wValue.c[LSB] | DESCR_FLAG_SVY;     
               putSph0 = Setup.wValue.c[LSB] | DESCR_FLAG_SPH0;    
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
			   break;
            case VENDOR_REQUEST_START_STREAMING:
			   doStream=1;
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
               break;
			case VENDOR_REQUEST_STOP_STREAMING: 
			   doStream=0;
			   doReset=1;
               POLL_WRITE_BYTE(E0CSR, rbSOPRDY); // Serviced Setup Packet.
...
//'	  if(hostInRequest) // This should not happen, got interrupt before completing packet. Something is wrong.
//'	  {
//'	     //TODO: Handle this also in the main loop
//'		 //TODO: Flush buffer
//'		 //TODO: Send zero packet
//'		 //POLL_WRITE_BYTE(EINCSR1, rbInSDSTL); //Send a stall(?)
//'      //hostInRequest=0;
//'      }
                                       // Ready to put new data on Fifo
      //'hostInRequest=1; //'maybe not needed at all?


//'      Fifo_Write(FIFO_EP1, EP1_PACKET_SIZE, (BYTE *)IN_PACKET);
//'      POLL_WRITE_BYTE(EINCSR1, rbInINPRDY);
                                        // Set In Packet ready bit, indicating fresh data on Fifo 1
//'      writePos = IN_PACKET + 1; //' Reset the buffer.
         //doStream=1; //debug
        //k++; //debug
