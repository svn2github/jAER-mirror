...
//	SYNCDELAY;
	...
	CKCON|=0x08; // T0M=1; // clock every CLK24/4=6MHz. Timer0 16 bit will overflow every 10.1ms
	SYNCDELAY;
	...
	led0 = 1;   // for rsv2 control, normal high
...
void TD_Poll(void){
	...
	if(in_enable) {                            // if IN transfers are enabled
		...
		if ( GPIFTRIG & 0x80 ) { // if GPIF interface IDLE, we finished the previous transaction of 128 events (hispeed)
				...
				TF0=0;
				TH0=0;
				TL0=1;	// reset timer0, because we just finished a transaction. we don't want to fire early transfer until 
...
#define VR_RSV2_CMD 0xbe // set the LED state
...
BOOL DR_VendorCmnd(void)
{
	...
	switch (SETUPDAT[1]){
		...
		case VR_RSV2_CMD: // sends 2 bytes following Robosapien V2's protocol
			{
				// led0 is high
				// commands starts with low signal for 8/1200s
				rsv2_cmd = SETUPDAT[2];
				rsv2_cmd |= SETUPDAT[3] << 8;
				rsv2_precmd = 1;
				rsv2_cyclesleft = 625; // 8/1200s ~= 666*10us, but tweaked to produce the correct timing
				rsv2_cmdidx = 12;
				led0 = 0;
				rsv2_sendcmd = 1;
				*EP0BUF=VR_RSV2_CMD;
...
void ISR_Timer1(void) interrupt 3 { // timer1 is interrupt 3 because vector is 0x1b and 3 codes this in C51
	timestampClock=0; // clock the timetamp counter
	_nop_();
	_nop_();
	_nop_();
	timestampClock=1;
	...
	if (rsv2_sendcmd == 1) {
		if (rsv2_cyclesleft-- == 0) {
			if (rsv2_precmd == 1) { // finished low signal
				rsv2_precmd = 0;
				if (rsv2_cmdidx != 0) {
					rsv2_cmdidx--;
					//if ((rsv2_cmd[rsv2_cmdidx>>3])^(rsv2_cmdidx & 0x7) == 1)
					if (rsv2_cmd&1 != 0) {
						rsv2_cyclesleft = 317; // 4/1200s 333
					} else {
						rsv2_cyclesleft = 80; // 1/1200s 83
				 	}
					rsv2_cmd = rsv2_cmd >> 1;
				} else {
					rsv2_sendcmd = 0;
