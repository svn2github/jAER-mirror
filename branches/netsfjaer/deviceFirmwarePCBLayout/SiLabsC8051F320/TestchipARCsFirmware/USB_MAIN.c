/* 	copyright Tobi Delbruck 28.1.05, 10.19.05, 17.1.06

This version is for the TestchipARCs PCB from patrick and includes a sync bit functionality for
...
sbit	BIAS_LATCH=P2^7;		// biasgen latch, // moved here as patch because we can't use the MISO as an output. this is AE bit 15 which is not used on tmpdiff128
	

		// bias latch was moved from P0^1 to P2^7 (AE15) because P0^1 was part of the hardware SPI clk/data. we are now using soft SPI, but we leave
		// biasLatch here because TestchipARCs only uses up to AE13
...
void sendSync(){
	...
	*AEPtr++=PCA0H;	// counter MSB, first down pipe
	*AEPtr++=tmp;	// counter LSB.
	AECounter++;
	EA=1;
