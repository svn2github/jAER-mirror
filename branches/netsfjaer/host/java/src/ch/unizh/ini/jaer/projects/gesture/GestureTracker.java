...
//    JFrame preferencesFrame;
    ...
    private float classSizeRatio=getPrefs().getFloat("GestureTracker.classSizeRatio",2);
    private boolean sizeClassificationEnabled=getPrefs().getBoolean("GestureTracker.sizeClassificationEnabled",true);
    ...
    protected float mixingFactor=getPrefs().getFloat("GestureTracker.mixingFactor",0.01f); // amount each event moves COM of cluster towards itself
    {setPropertyTooltip("mixingFactor","how much cluster is moved by an event and its distance from the present locatoins");}
    protected float velocityMixingFactor=getPrefs().getFloat("GestureTracker.velocityMixingFactor",0.01f); // mixing factor for velocity computation
    {setPropertyTooltip("velocityMixingFactor","how much cluster velocity estimate is updated by each event");}
    
    private float surround=getPrefs().getFloat("GestureTracker.surround",2f);
    {setPropertyTooltip("surround","the radius is expanded by this ratio to define events that pull radius of cluster");}
    private boolean dynamicSizeEnabled=getPrefs().getBoolean("GestureTracker.dynamicSizeEnabled", false);
    {setPropertyTooltip("dynamicSizeEnabled","size varies dynamically depending on cluster events");}
    private boolean dynamicAspectRatioEnabled=getPrefs().getBoolean("GestureTracker.dynamicAspectRatioEnabled",false);
    {setPropertyTooltip("dynamicAspectRatioEnabled","aspect ratio depends on events as well");}
    private boolean pathsEnabled=getPrefs().getBoolean("GestureTracker.pathsEnabled", true);
    {setPropertyTooltip("pathsEnabled","draw paths of clusters over some window");}
    private boolean colorClustersDifferentlyEnabled=getPrefs().getBoolean("GestureTracker.colorClustersDifferentlyEnabled",false);
    ...
    private boolean useOnePolarityOnlyEnabled=getPrefs().getBoolean("GestureTracker.useOnePolarityOnlyEnabled",false);
    {setPropertyTooltip("useOnePolarityOnlyEnabled","use only one event polarity");}
    private boolean useOffPolarityOnlyEnabled=getPrefs().getBoolean("GestureTracker.useOffPolarityOnlyEnabled",false);
    {setPropertyTooltip("useOffPolarityOnlyEnabled","use only OFF events, not ON - if useOnePolarityOnlyEnabled");}
    private float aspectRatio=getPrefs().getFloat("GestureTracker.aspectRatio",1f);
    {setPropertyTooltip("aspectRatio","default (or starting) aspect ratio, taller is larger");}
    private float clusterSize=getPrefs().getFloat("GestureTracker.clusterSize",.2f);
    {setPropertyTooltip("clusterSize","size (starting) in chip pixels");}
    protected boolean growMergedSizeEnabled=getPrefs().getBoolean("GestureTracker.growMergedSizeEnabled",false);
    ...
    private boolean showVelocity=getPrefs().getBoolean("GestureTracker.showVelocity",true); // enabling this enables both computation and rendering of cluster velocities
    ...
    private boolean classifierEnabled=getPrefs().getBoolean("GestureTracker.classifierEnabled",false);
    {setPropertyTooltip("classifierEnabled","colors clusters based on single size metric");}
    private float classifierThreshold=getPrefs().getFloat("GestureTracker.classifierThreshold",0.2f);
    {setPropertyTooltip("classifierThreshold","the threshold for classification");}
    private boolean showAllClusters=getPrefs().getBoolean("GestureTracker.showAllClusters",false);
    {setPropertyTooltip("showAllClusters","shows all clusters, not just those with sufficient support");}
    private boolean useNearestCluster=getPrefs().getBoolean("GestureTracker.useNearestCluster",false); // use the nearest cluster to an event, not the first containing it
    {setPropertyTooltip("useNearestCluster","shows all clusters, not just those with support");}
    private boolean clusterLifetimeIncreasesWithAge=getPrefs().getBoolean("GestureTracker.clusterLifetimeIncreasesWithAge",false);
    ...
    private boolean highwayPerspectiveEnabled=getPrefs().getBoolean("GestureTracker.highwayPerspectiveEnabled",false);
    {setPropertyTooltip("highwayPerspectiveEnabled","Cluster size depends on perspective location; mouse click defines horizon");}
    
    private int thresholdEventsForVisibleCluster=getPrefs().getInt("GestureTracker.thresholdEventsForVisibleCluster",10);
    {setPropertyTooltip("thresholdEventsForVisibleCluster","Cluster needs this many events to be visible");}
    
    private int clusterLifetimeWithoutSupportUs=getPrefs().getInt("GestureTracker.clusterLifetimeWithoutSupport",10000);
...
//        prefs.addPreferenceChangeListener(this);
    ...
    public void initFilter() {
        initDefaults();
        chipSizeX = chip.getSizeX();
        chipSizeY = chip.getSizeY();
        //defaultClusterRadius=(int)Math.max(chip.getSizeX(),chip.getSizeY())*getClusterSize();
        defaultClusterRadius=(int)Math.max(chipSizeX,chipSizeY)*getClusterSize();  //added PW
    }
    
    private void initDefaults(){
        initDefault("GestureTracker.clusterLifetimeWithoutSupport","10000");
        initDefault("GestureTracker.maxNumClusters","1");   //edited PW
        initDefault("GestureTracker.clusterSize","0.15f");
...
//        if(clusters.size()>beforeMergeCount) throw new RuntimeException("more clusters after merge than before");
    ...
    public String toString(){
        ...
        String s2="GestureTracker with "+s+" clusters ";
        return s2;
    }
...
////        }
    ...
    public void setClusterLifetimeWithoutSupportUs(final int clusterLifetimeWithoutSupport) {
        ...
        getPrefs().putInt("GestureTracker.clusterLifetimeWithoutSupport", clusterLifetimeWithoutSupport);
    }
    ...
    public void setClusterSize(float clusterSize) {
        ...
        getPrefs().putFloat("GestureTracker.clusterSize", clusterSize);
    }
    
    private int maxNumClusters=getPrefs().getInt("GestureTracker.maxNumClusters",1);  //changed PW
    ...
    public void setMaxNumClusters(final int maxNumClusters) {
        ...
        getPrefs().putInt("GestureTracker.maxNumClusters", maxNumClusters);
    }
...
//    }
    ...
    public void setThresholdEventsForVisibleCluster(final int thresholdEventsForVisibleCluster) {
        ...
        getPrefs().putInt("GestureTracker.thresholdEventsForVisibleCluster", thresholdEventsForVisibleCluster);
    }
    ...
    public void setHighwayPerspectiveEnabled(boolean highwayPerspectiveEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.highwayPerspectiveEnabled",highwayPerspectiveEnabled);
    }
    ...
    public void setMixingFactor(float mixingFactor) {
        ...
        getPrefs().putFloat("GestureTracker.mixingFactor",mixingFactor);
    }
    ...
    public void setSurround(float surround){
        ...
        getPrefs().putFloat("GestureTracker.surround",surround);
    }
    ...
    public void setPathsEnabled(boolean pathsEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.pathsEnabled",pathsEnabled);
    }
    ...
    public void setDynamicSizeEnabled(boolean dynamicSizeEnabled){
        ...
        getPrefs().putBoolean("GestureTracker.dynamicSizeEnabled",dynamicSizeEnabled);
    }
    ...
    public void setColorClustersDifferentlyEnabled(boolean colorClustersDifferentlyEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.colorClustersDifferentlyEnabled",colorClustersDifferentlyEnabled);
    }
    ...
    public void setUseOnePolarityOnlyEnabled(boolean useOnePolarityOnlyEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.useOnePolarityOnlyEnabled",useOnePolarityOnlyEnabled);
    }
    ...
    public void setUseOffPolarityOnlyEnabled(boolean useOffPolarityOnlyEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.useOffPolarityOnlyEnabled",useOffPolarityOnlyEnabled);
    }
...
//    }
    ...
    public void setGrowMergedSizeEnabled(boolean growMergedSizeEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.growMergedSizeEnabled",growMergedSizeEnabled);
    }
    ...
    public void setVelocityMixingFactor(float velocityMixingFactor) {
        ...
        getPrefs().putFloat("GestureTracker.velocityMixingFactor",velocityMixingFactor);
    }
    
    public void setShowVelocity(boolean showVelocity){
        this.showVelocity = showVelocity;
        getPrefs().putBoolean("GestureTracker.showVelocity",showVelocity);
    }
    ...
    public void setAspectRatio(float aspectRatio) {
        ...
        this.aspectRatio = aspectRatio;
        getPrefs().putFloat("GestureTracker.aspectRatio",aspectRatio);
    ...
    public void setClassifierEnabled(boolean classifierEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.classifierEnabled",classifierEnabled);
    }
    ...
    public void setClassifierThreshold(float classifierThreshold) {
        ...
        getPrefs().putFloat("GestureTracker.classifierThreshold",classifierThreshold);
    }
    ...
    public void setShowAllClusters(boolean showAllClusters) {
        ...
        getPrefs().putBoolean("GestureTracker.showAllClusters",showAllClusters);
    }
    ...
    public void setDynamicAspectRatioEnabled(boolean dynamicAspectRatioEnabled) {
        ...
        getPrefs().putBoolean("GestureTracker.dynamicAspectRatioEnabled",dynamicAspectRatioEnabled);
    }
    ...
    public void setUseNearestCluster(boolean useNearestCluster) {
        ...
        getPrefs().putBoolean("GestureTracker.useNearestCluster",useNearestCluster);
    }
    ...
    public void setClusterLifetimeIncreasesWithAge(boolean clusterLifetimeIncreasesWithAge) {
        ...
        getPrefs().putBoolean("GestureTracker.clusterLifetimeIncreasesWithAge",clusterLifetimeIncreasesWithAge);
